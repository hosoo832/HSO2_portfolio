...
print("\n--- [BACKFILL] 과거 데이터 일괄 등록 스크립트 시작 (v70.33 - ECOS 통합) ---")
...
def run_backfill():
    ...
    try:
        ...
        # [v70.34] 등락률은 ffill 전(원본 NaN 유지)에 계산해야 휴장일 0% 안 나옴
        #   - fill_method=None 으로 pct_change 의 자동 ffill 을 끔
        #   - 이후 chg 컬럼들도 ffill 해서 휴장일은 직전 거래일 값으로 유지 (cron 동작과 일치)

        # 1. 등락률 계산 (원본 가격에서, NaN 유지)
        df_final['KOSPI_chg_pct']    = df_final['KOSPI_price'].pct_change(fill_method=None)
        df_final['KOSDAQ_chg_pct']   = df_final['KOSDAQ_price'].pct_change(fill_method=None)
        df_final['SP500_chg_pct']    = df_final['SP500_price'].pct_change(fill_method=None)
        df_final['NASDAQ_chg_pct']   = df_final['NASDAQ_price'].pct_change(fill_method=None)
        df_final['SHANGHAI_chg_pct'] = df_final['SHANGHAI_price'].pct_change(fill_method=None)
        df_final['NIKKEI_chg_pct']   = df_final['NIKKEI_price'].pct_change(fill_method=None)
        df_final['DAX_chg_pct']      = df_final['DAX_price'].pct_change(fill_method=None)
        df_final['USDKRW_chg_pct']   = df_final['USDKRW_price'].pct_change(fill_method=None)
        df_final['USD_IDX_chg_pct']  = df_final['USD_IDX_price'].pct_change(fill_method=None)
        df_final['WTI_chg_pct']      = df_final['WTI_price'].pct_change(fill_method=None)
        df_final['GOLD_chg_pct']     = df_final['GOLD_price'].pct_change(fill_method=None)
        df_final['BTC_chg_pct']      = df_final['BTC_price'].pct_change(fill_method=None)
        df_final['VIX_chg_pct']      = df_final['VIX_price'].pct_change(fill_method=None)
        ...
        # 자금흐름 등락률 계산 (Customer_Deposit/Credit_Balance — 비어있어도 안전)
        if 'Customer_Deposit_value' in df_final.columns:
            df_final['Customer_Deposit_chg_pct'] = df_final['Customer_Deposit_value'].pct_change(fill_method=None)
        if 'Credit_Balance_value' in df_final.columns:
            df_final['Credit_Balance_chg_pct'] = df_final['Credit_Balance_value'].pct_change(fill_method=None)

        # 2. 가격/지수 ffill (휴장일에 직전 거래일 가격 전파)
        df_final[cols_to_ffill] = df_final[cols_to_ffill].ffill()

        # 3. 등락률 ffill (휴장일에 직전 거래일 변동률 유지) + 시작 부분 NaN 만 0
        ...
        # [v70.34] Volume(거래대금)도 ffill: 휴장일에 직전 거래일 값 유지 (cron 동작과 일치)
        df_final['KOSPI_volume']  = df_final['KOSPI_volume'].ffill().fillna(0)
        df_final['KOSDAQ_volume'] = df_final['KOSDAQ_volume'].ffill().fillna(0)
