...
| 파일 | 역할 |
|---|---|
| `Dashboard.py` | 대시보드 메인. 6개 뷰: 전체 / 멘토 포폴 / HS 포폴 / 장중 실시간 / 작전 일지 / 🎯 은퇴 로드맵 |
| `retirement_roadmap.py` | 은퇴 로드맵 뷰 (Dashboard "🎯 은퇴 로드맵"이 호출하는 독립 모듈). 목표 은퇴자산·필요수익률·수익률3종(나/와이프/합산)·연도별 자산 로드맵. 설계결정 #13 |

### GitHub Actions
| 파일 | 역할 |
|---|---|
| `.github/workflows/daily-market.yml` | 매일 06:10 KST (cron-job.org 트리거). update_market_data.py → ma_touch.py 순차 실행 |
...
### 13. 은퇴 로드맵 뷰 (`retirement_roadmap.py`, 2026-06-22)
Dashboard 6번째 뷰 "🎯 은퇴 로드맵". 로직은 별도 모듈 `retirement_roadmap.py` 의 `render(df_dashboard, df_perf, now_kst)` 에 격리 (Dashboard.py 는 radio 옵션 + 호출 블록만 수정, 본체 영향 0). 호섭 한국나이 50/55세 은퇴 목표자산 추적·점검.

**모델 (실질 = 오늘 구매력, 억원):** 거주 버킷(전세/반전세 보증금 → 미래 과천 아파트, 실질 불변) + 투자 버킷(주식+경매, 복리). 목표 ≈40억 = 과천 84㎡ 22억 + (생활비 5,400 + 여행 1,750만)/SWR 4%. 86년생 → 50세=2035(9년)/55세=2040(14년), 근로 추가납입 0. 핵심 결론: 추가납입 0이면 50세는 비현실(명목 14~21%), **55세+반전세가 현실권(11%대)**.

**화면 4블록:**
- 🏅 **수익률 3종** — 🧑 나(주식, 와이프송금 복원) / 👩 와이프(경매, 원금 대비) / 🤝 합산(투자버킷). KPI 10/15/20% 신호등. 원칙: **은퇴 도달 = 합산 자산 '금액', 실력 평가 = 각자 수익률**.
- ① **올해 성적**(막대): 올해까지 목표 vs 합산 투자 YTD.
- ② **오늘 기준 매년 필요수익률**(막대): 연초 기준 vs 오늘 기준 + 올해 성과로 낮춘 폭. 매일 자산 따라 갱신.
- ④ **연도별 자산 로드맵**(차트+표): 나이·연도별 목표 궤도(필요수익률로 매년 가야 할 자산) vs 예상(@기대수익률).

**DEFAULTS 핵심값:** `auction=3.7`(경매=와이프 4억 매도 세후 실현가) / `auction_cost=2.48`(경매 원금) / `wife_funding=0.4823`(와이프 누적 송금 ≈4,823만, 2025-12-03~). 주식자산은 dashboard_data 자동, 전세·경매·목표아파트는 슬라이더 입력. YTD는 연율 환산 안 하고 누적 그대로(호섭 선호).

**중요 개념:** 와이프 경매 송금(4,823만)은 주식→경매 **내부이동** → 합산 수익률엔 중립(개별로만 주식 과소·경매 과대). 화면 'YTD +40%'는 부풀린 게 아니라 주식+경매 합산 순수 운용수익. 본인 실제 실력은 performance_summary TWR.

> ⚠️ **배포 주의 — OneDrive 이중화 함정 (2026-06-22 대참사):** MyPortfolio + `.git` 이 OneDrive 안이라, Cowork(Claude)가 bash·파일도구로 쓴 파일이 **사용자 로컬 git working tree 에 안 닿음**(`git status` 가 clean, sync 해도 옛 버전, 최악엔 파일 중간 잘려 SyntaxError). 복구 절차: ① 정상 파일 받아서 → ② OneDrive **완전 "종료"**(일시정지 X, 트레이→종료) → ③ 탐색기로 덮어쓰기 → ④ `git status` 에 modified 뜨면 `git add retirement_roadmap.py` + commit + push → ⑤ OneDrive 재시작. GitHub 검증은 `raw.githubusercontent.com/hosoo832/HSO2_portfolio/<커밋해시>/retirement_roadmap.py` 로 (main URL 은 CDN 캐시라 옛것 보임). **근본해결: 리포를 OneDrive 밖(예: `C:\dev\`)으로 옮기면 이 문제 사라짐.**
...
> 마지막 갱신: **2026-06-22** (은퇴 로드맵 뷰 `retirement_roadmap.py` 신설 — 설계결정 #13; OneDrive 이중화 배포 함정 기록)
>
> ### 갱신 이력
...
> - 2026-06-22: **은퇴 로드맵 뷰 신설** (`retirement_roadmap.py`, 설계결정 #13) — Dashboard 6번째 뷰. 거주(전세/반전세)+투자(주식+경매) 버킷, 목표 ≈40억(과천 22 + 인출재원 18), 화면 4블록(🏅 수익률 3종 나/와이프/합산 · ① 올해 성적 · ② 오늘 기준 필요수익률 · ④ 연도별 자산 로드맵, 다 막대/표). 경매=와이프 세후 실현 3.7억(원금 2.48), 와이프 송금 4,823만=주식↔경매 내부이동(합산수익률 중립·개별만 왜곡). ⚠️ **OneDrive 이중화로 배포 수십 번 실패** 후 'OneDrive 완전 종료→탐색기 덮어쓰기→git add/commit/push' 흐름으로 해결 — 상세는 설계결정 #13. 근본해결(미완): 리포를 OneDrive 밖으로.
