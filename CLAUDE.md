...
### 10. Naver chg_pct override + 휴장일 판별 (5/18 사고)
Naver 가 일/공휴일 다음 cron 시점 (예: 월요일 07시) 에 잘못된 direction 줘서 chg 가 +6.97% 등 망가지는 사고 있었음.
- **fix**: Naver traded_date 가 yesterday_kst 와 **정확히 같을 때만** fallback (엄격 비교)
- 어제가 휴장이거나 Naver 가 비정상 timestamp 주면 스킵 → yfinance series.dropna() 로 chg 계산
