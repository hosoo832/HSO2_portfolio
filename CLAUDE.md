...
| 시트 | 주요 컬럼 | 비고 |
|---|---|---|
| `raw_domestic` | 계좌번호, 거래일자, 거래종류, 적요명, 종목코드, 종목명, 거래수량, 정산금액 | 증권사 거래내역 붙여넣기. **보통매매 매수/매도 행은 main.py가 무시**(raw_체결 담당) — 입출금·배당·환전·입고·분할만 사용 |
| `raw_체결` | 계좌번호, 체결일, 종목코드, 종목명, 매매구분, 체결수량, 체결평균단가, 정산금액 | **국내 시장매매(보통매매 매수/매도)**. 체결내역 기반, 체결일 기준 — 당일 확보. 정산금액 비우면 main.py가 계산. 미래에셋 퇴직연금 매매는 여기 수기입력 |
| `raw_체결_키움` | (키움 체결내역 raw 2줄/건 그대로) + A열 계좌번호 | 키움 체결내역을 **가공 없이 그대로** 붙여넣는 입력 시트. main.py·Dashboard가 읽으며 `flatten_kiwoom_chey`로 자동 2줄→1줄 평탄화·통합 |
...
### 11. raw_체결 시스템 — 국내 매매를 체결내역 기반으로 (2026-05-26)
...
1. 증권사 거래 입력:
   - **국내 매매** → `raw_체결` (키움 체결내역 / 미래에셋 퇴직연금은 수기). 정산금액 칸은 비워도 됨
   - **입출금·배당·환전 등** → `raw_domestic` 에 거래내역 붙여넣기 (매매 행 섞여 있어도 무시되니 OK)
   - **해외 거래** → `raw_international`
...
> 마지막 갱신: **2026-05-26** (raw_체결 시스템 — 국내 매매를 체결내역 기반으로 전환)
>
> ### 갱신 이력
...
> - 2026-05-26: **raw_체결 시스템** — 국내 보통매매 매수/매도를 체결내역(raw_체결 시트) 기반으로 전환. 소스 분리(매매=raw_체결, 그 외=raw_domestic, dedup 불필요), `transform_chey` 신설, `transform_domestic` 에 `exclude_market_trades` 옵션, 분할체결 통합, 수수료 추정, 거래내역↔체결 감사. main.py(현금계산 포함)·Dashboard 작전일지(직전 영업일 import) 반영. `migrate_chey.py`/`verify_chey.py` 추가. 설계결정 #11.
